...
class Wire {
    ...
    markAsVisited(coordinate) {
        ...
        this.m_lengths[serializedCoordinate] = this.m_currentLength;
...
const calculateWireDistance = (coordinate, wireObj, wireObj2) => {
    return wireObj.m_lengths[coordinate] + wireObj2.m_lengths[coordinate];
}
...
const naive = (input1, input2) => {
    ...
    const distances = result.map((one) => {
        return calculateWireDistance(one, wireOne, wireTwo);
    }).filter((one) => {
        return one !== 0;
    });
...
const compareOverlapsImmediately = (input1, input2) => {
    ...
    const distances = wireTwo.m_overlaps.map((one) => {
        return calculateWireDistance(JSON.stringify(one), wireOne, wireTwo);
    }).filter((one) => {
        return one !== 0;
    });
...
compareOverlapsImmediately(wire1, wire2);
