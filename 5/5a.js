...
const testData = [1002,4,3,4,33];
const testData2 = [1101,100,-1,4,0];
...
class Add extends OpCodeInstruction {
    ...
    process = (data, positionFrom, paramTypes) => {
        this.m_data = data;
        const value1 = getParamType(paramTypes, 0) === 0 ? data[data[positionFrom + 1]] : data[positionFrom + 1];
        const value2 = getParamType(paramTypes, 1) === 0 ? data[data[positionFrom + 2]] : data[positionFrom + 2];
...
class Output extends OpCodeInstruction {
    m_params = 1;
    process = (data, positionFrom, paramTypes) => {
        getParamType(paramTypes, 0) === 0 ? output(data[data[positionFrom + 1]]) : output(data[positionFrom+1]);
    }
}
...
const process = (data, positionFrom) => {
    ...
    return process(data, positionFrom + 1 + op.m_params);
}
